...
```{r}
soccer_fouls <- read_csv ("https://raw.githubusercontent.com/dwillis/NCAAWomensSoccerData/refs/heads/main/data/ncaa_womens_soccer_matchstats_2024.csv")
```
### Task 3: Create a new column called `goal_diff` that calculates the difference between the two teams' goals.
```{r}
soccer_fouls <- soccer_fouls |> mutate(goal_diff = team_score - opponent_score)
```
...
```{r}
soccer_fouls |> summarize(correlation = cor(fouls, goal_diff, method="pearson"))
```
...
The correlation value and the scatter plot show a very weak positive correlation between the goal differation and the number of fouls.
