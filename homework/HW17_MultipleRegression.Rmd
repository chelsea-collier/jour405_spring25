...
Income and pct white show strong positive correlations with voter turnout. Pct bachelors and median age are also moderately positively correlated. Pct unemployed shows a negative correlation with turnout and population shows little correlation. 
...
This model is useful. The model explains about 80.3% of the turnout variation and is statisticlly signifigant. Income, median age, and pct of white residents are all positively associated with turnout. Population, unemployment and college education are all not statisticly signifigant. The residuals are centered w/ moderate spread so the model is a good fit.
...
I found that by removing population, median age and unemployment, made the model a slightly better fit. When I used only education and race  my R val was slightly lower and it was also lower when I removed population and race. But all remained pretty strong models even with the changes.
...
# Model 3
model3 <- lm(turnout ~ pct_bachelors_higher + pct_white, data = md_turnout)
summary(model3)
...
The analysis can be used explain why voter turnout varies so much across Maryland counties. For example, counties with higher incomes, older populations, and more white residents tend to vote at higher rates. The story could focus on the inequality of turnouts and how economic and demographic factors effect civic participation and efforts to address this. 
